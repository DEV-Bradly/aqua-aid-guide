### 🌊 Aqua Aid Guide – Clean Water & Sanitation (SDG 6) ###

Aqua Aid Guide is a web-based platform designed to promote awareness, monitor water quality, and support communities in achieving the United Nations Sustainable Development Goal 6 (Clean Water & Sanitation)


LIVE🔴🚨
## 🔗 Live Demo:  https://aqua-aid-guide-7ldw2s564-bradly-kiptanui-s-projects.vercel.app/ ##

LIVE🔴LIVE🔴
🔗 Live Pitch Deck:
**Pitch deck of the project : **https://www.canva.com/design/DAG3ldeBt6g/PTYeB2qJZf7Cra5cPl6l4Q/edit?utm_content=DAG3ldeBt6g&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton**


 ## 📖 Overview  ##

Access to clean water and proper sanitation is a global challenge — over 2.2 billion people lack safe water.
Aqua Aid Guide leverages modern web technology to:

✅Educate users about water quality and safety.

✅Analyze basic water test data.

✅Encourage sustainable water management practices.


 ## 💧 Features ##

🧪 Water Quality Checker – Input and analyze pH, turbidity, and temperature data.

📚 Education Hub – Learn about water safety, treatment, and conservation.

💧 Usage Calculator – Track daily water use and identify waste.

🗂️ Data Storage – Save and retrieve water testing records.

📱 Responsive UI – Works smoothly on mobile and desktop.

🔒 Safe & Accessible – Built with user accessibility and data security in mind.


## 🧠 SDG 6 Alignment ##

Aqua Aid Guide directly supports UN SDG 6: Clean Water and Sanitation, by:

Promoting community awareness. 📢📢

Encouraging responsible usage.🌊🌊

Empowering youth and institutions to take local action.🌎🌎



## 🧩 Technology Stack  ##
Tool	Purpose
⚡ Vite --
	Fast development build tool
⚛️ React --
	Frontend UI framework
💻 TypeScript  --
	Type-safe JavaScript
🎨 Tailwind CSS  --
	Modern styling framework
🧱 shadcn/ui --
	Prebuilt accessible UI components

  
## 🚀 Future Roadmap ##
Timeline	Goal
Q1 2025	IoT water sensor integration
Q2 2025	Mobile PWA version                                                                    
Q3 2025	Regional dashboard for analytics
2026	AI-driven water quality alerts


##  QR CODE  SCAN  ##
![sdg QRcode](https://github.com/user-attachments/assets/15c5f19d-ec8a-4434-b5fd-e912676ec3ea)


 ## 🧭 How It Works ##

1️⃣ Enter local water test data (pH, turbidity, temperature).
2️⃣ The system analyzes and provides real-time feedback.
3️⃣ Access educational content to understand your results.
4️⃣ Track daily water usage through the calculator feature.

## 💼 Target Users ##

🎓 Schools – Water education programs.

🌱 NGOs – Awareness and outreach.

🏡 Households – Monitoring domestic water.

💼 Governments – Data insights and reporting.

 ##  FOLLOW THE  STEPS TO ##
 
```sh
# Step 1: Clone the repository using the project's Git URL.
git clone 

# Step 2: Navigate to the project directory.
cd aqua-aid-guide
/

# Step 3: Install the necessary dependencies.
npm i

# Step 4: Start the development server with auto-reloading and an instant preview.
npm run dev
```
## SAMPLE OF JPNG PROJECT  ##
 ** DASHBOARD **
 
 ![Dashbord sdg6](https://github.com/user-attachments/assets/c15d24b5-a389-4676-880e-086540f32648)     ## 

 
FIND ME ON GITHUB

https://github.com/DEV-Bradly

 ## 🧾 License ##

This project is open-source under the MIT License
.





